---
layout: default
title: Home
---

<div class="home-grid">
  <div>
    <img src="{{ '/picture/image.png' | relative_url }}" alt="Asker Nygaard Christensen" class="profile-photo">
  </div>
  <div>

<p>I am a PhD student at the <a href="https://www.economics.ku.dk/">University of Copenhagen, Department of Economics</a> (2022-2026).<br>
My supervisors are Daphné Skandalis and Nikolaj Harmon.<br>
In August 2024–May 2025, I visited Michèle Belot at Cornell University.</p>

<h3>Research interests</h3>
<ul class="interests-list">
  <li>Labor economics</li>
  <li>Job search</li>
  <li>Applied econometrics</li>
  <li>Text analysis / NLP</li>
</ul>

<div class="profile-links">
  <a href="https://researchprofiles.ku.dk/da/persons/asker-nygaard-christensen/">Department profile</a>
  <a href="https://orcid.org/0000-0002-5939-1427">ORCID</a>
  <a href="https://github.com/AskerNC">GitHub</a>
</div>

  </div>
</div>

<div class="name-note">
I am getting married in October 2026, so in order to minimize future confusion, my future publications will be under the name <strong>Asker Nygaard David</strong> (previous name was Asker Nygaard Christensen).
</div>

<div class="contact-box">
  <p><strong>Address</strong></p>
  <p>Department of Economics,<br>
  University of Copenhagen<br>
  Øster Farimagsgade 5, Building 26<br>
  1353 Copenhagen K, Denmark</p>
  <p>Office: 26.1.03, 1st floor</p>
  <p>Email: <a href="mailto:hms467@econ.ku.dk">hms467@econ.ku.dk</a></p>
</div>
